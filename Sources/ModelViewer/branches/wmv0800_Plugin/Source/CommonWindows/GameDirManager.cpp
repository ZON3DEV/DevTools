...
GameDirManager::GameDirManager(QWidget *parent) : QDialog(parent), ui_GameDirManager(new Ui::GameDirManager)
{
	// -= Setup Window =-
	ui_GameDirManager->setupUi(this);
	/*
	// Icons
...
GameDirManager::~GameDirManager()
{
	delete ui_GameDirManager;
}
void GameDirManager::init(){
	// Update List
	UpdateList();
}
...
void GameDirManager::saveDir(st_WoWDir dir, bool noUpdate = false)
{
	/*
	// Get the Group Name
...
void GameDirManager::on_GameDirManager_bDirAdd_clicked(){
	/*
	QLOG_INFO() << "Attempting to add a new WoWDir...";
	// Directory chooser
	QString dir = QFileDialog::getExistingDirectory(this,tr("Choose the WoW Directory you wish to add."),sWMVSettings.value("GameDirManagerLastDir").toString());
	if (dir == QString()){
	...
	sWMVSettings.setValue("GameDirManagerLastDir", dDir.absolutePath());
	sWMVSettings.sync();
...
void GameDirManager::on_GameDirManager_bDirEdit_clicked(){
	Window_GameDirManagerEdit.init();
	Window_GameDirManagerEdit.show();
}

// Make the Currently selected directory the Current Directory.
void GameDirManager::on_GameDirManager_bDirMakeCurrent_clicked(){
	/*
...
void GameDirManager::ListOffset(int offset){
/*
	int num = -1;
	for (size_t i=0;i<List->count();i++){
		QListWidgetItem *a = List->item((int)i);
...
void GameDirManager::on_GameDirManager_bDirUp_clicked(){
	QLOG_INFO() << "Moving Directory Up 1 Slot...";
	ListOffset(-1);
}

// Move directory down the list
void GameDirManager::on_GameDirManager_bDirDown_clicked(){
	QLOG_INFO() << "Moving Directory Down 1 Slot...";
	ListOffset(1);
}

// Called when someone clicks the "Delete All" button
void GameDirManager::on_GameDirManager_bDirDeleteAll_clicked(){
	/*
	...
	if (choice == QMessageBox::Yes){
		...
		g_WMV->UnLoadWoW();
		UpdateList();
		SettingsList.insert("GameDirManagerLastDir","None");
...
// Prepare the List
void GameDirManager::SetupList()
{
	// Copy settings to our local list
	List = ui_GameDirManager->GameDirManager_List;
	
	// Change various settings to fit our needs
	
	// Re-Apply the settings to the visible list.
	ui_GameDirManager->GameDirManager_List = List;
}

// Update the List with our items!
void GameDirManager::UpdateList()
{
	/*
	ReadWoWDirList();
	QLOG_INFO() << "Updating GameDirManager List...";
	// Grab selected item
	...
	QLOG_INFO() << "Sorting GameDirManager List...";
	...
	// Apply the new data
	ui_GameDirManager->GameDirManager_List = List;
	QLOG_INFO() << "Finished Updating GameDirManager List!";
	...
	for (ssize_t i=iface.count()-1;i>=0;i--){
		...
		if (QFile(a).exists()){
			if (QFile(a+".lock").exists()){
				...
				if (!mpq->OK)
					continue;
				tocarch.push_back(cArchiveHandle(mpq->FileInfo,mpq->getHANDLE(),mpq->isPatchFile));
			}
		}
	}
...
void GameDirManager::ReOrderList(UnReal in_pos, Real offset)
{
	/*
...
GameDirManager_Edit::GameDirManager_Edit(QWidget *parent) : QDialog(parent), ui_GameDirManager_Edit(new Ui::GameDirManager_Edit)
{
	ui_GameDirManager_Edit->setupUi(this);
}

GameDirManager_Edit::~GameDirManager_Edit()
{
	delete ui_GameDirManager_Edit;
}

void GameDirManager_Edit::init()
{

}

void GameDirManager_Edit::on_btn_Okay_clicked()
{
	// Save the data
